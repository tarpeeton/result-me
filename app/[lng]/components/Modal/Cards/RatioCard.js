...
export default function RatioCard({ item, handleRatioChange, value: parentValue }) {
  ...
  const {lng}  = useParams()  // Обновление значения при изменении родительского значения (при переключении калькулятора)
  useEffect(() => {
  ...
  return (
    <div className="ratio-card bg-white p-6 rounded-[20px] flex flex-col justify-between gap-14 relative">
      <div>
        <h3 className="text-2xl font-bold mb-2">{item.title[lng]}</h3>
        <p className="text-gray-600 mb-6">{item.descriptions[lng]}</p>
      </div>
