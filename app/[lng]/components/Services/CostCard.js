...
const CostCard = () => {
  return (
    <div className='bg-white100 px-[24px] pt-[30px] rounded-[30px] flex flex-col 3xl:flex-col  3xl:p-[40px] mx-[16px] mt-[20px] 3xl:mx-[30px]'>
      ...
      <div className='rounded-[30px] mt-[20px] relative 3xl:flex 3xl:flex-row 3xl:gap-[25px]'>
        {/* Mapping through CostCardData array */}
        {CostCardData.map((card, index) => (
          <div key={index} className='relative border px-[24px] py-[30px] border-[#F0F0F0] rounded-[30px] mt-[20px] 3xl:w-[33%] pb-[150px]'>
            <div>
            ...
            <div className='mt-[20px]'>
              ...
              {card.description.split('\n').map((line, lineIndex) => (
                <p key={lineIndex} className='text-[15px] mdl:text-[18px] text-[#454545] font-normal'>
                  {line}
                </p>
              ))}
            </div>
              ...
              {card.services.map((service, serviceIndex) => (
                <div key={serviceIndex} className='flex flex-row gap-[12px]'>
                  ...
                  <div className='flex items-center'>
                    <p className='text-[#010101] text-[15px] font-medium'>{service}</p>
                  </div>
                </div>
              ))}
            </div>
